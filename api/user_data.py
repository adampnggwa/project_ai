...
async def update_account(meta: EditProfileRequest, access_token: str = Header(...)):
    ...
    await user.save()
    response = pesan_response(email=user.email, message='Account updated successfully')
    return JSONResponse(response, status_code=201)

@router.get('/get-account')
async def get_account(access_token: str = Header(...)):
    validasi = await apakahAccessTokenValid(access_token=access_token)
    if validasi['status'] is False:
        raise HTTPException(status_code=401, detail=validasi['keterangan'])
    else:
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    return JSONResponse(content=await response_user(user=user), status_code=200)
